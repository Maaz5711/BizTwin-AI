...
app.add_middleware(
    ...
    "https://biz-twin-ai.vercel.app",
    "https://biz-twin-phrjefcza-maaz5711s-projects.vercel.app",
    "https://biz-twin-ai-git-main-maaz5711s-projects.vercel.app",
],
...
async def chat(body: ChatRequest) -> dict:
    ...
    print("\n========== CHAT REQUEST ==========")
    print("USER QUESTION:", body.question)

    if not twin.is_ready:
        print("ERROR: Business twin not ready")
        raise HTTPException(
            status_code=503,
            detail="No data loaded yet. Please upload CSV files first.",
        )
    ...
    try:
        ...
        print("AI IMPORT ERROR:", exc)

        return {
            "answer": (
                f"AI module not connected. "
                f"Revenue: ${kpi_context['revenue']:,.2f}, "
                f"Expenses: ${kpi_context['expenses']:,.2f}, "
                f"Profit: ${kpi_context['profit']:,.2f}"
            )
        }
